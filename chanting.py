...
scripture = f'이시 무진의보살 즉종좌기 편단우견 합장향불 이작시언. “세존, 관세음보살 이하인연 명관세음”\n\
불고 무진의보살.\n\
...
pyautogui.press('"')
os.system('tmux send-keys "python3 asciianim.py" Enter')

def write():
    typewriter(scripture * 5)

def say():
    i = 0
    if i == 0:
        i += 1
        subprocess.call(["afplay", "moktak.wav"]) #처음 한번만 
    os.system("say {}".format(scripture))
...
def sound():
    ...
    while True:
        i += 1
        time.sleep(60) # 1분 마다
        subprocess.call(["afplay", "moktak.wav"])
        print(f"Debug sound {i}\n")
        if i == 5:
            break
...
if __name__ == "__main__":
    ...
    print(f"Debug {proc.name}\n")

    proc = mp.Process(name="Subprocess1", target=say)
    proc.start()
    print(f"Debug {proc.name}\n")

    proc = mp.Process(name="Subprocess2", target=sound)
    proc.start()
    print(f"Debug {proc.name}\n")

    proc = mp.Process(name="Subprocess3", target=write)
    proc.start()
    print(f"Debug {proc.name}\n")
